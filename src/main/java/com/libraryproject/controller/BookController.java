...
public class BookController {
	...
	@PostMapping
	public void register(@RequestBody BookDTO bookDto) {
		Book book = modelMapper.map(bookDto, Book.class);

		Author author = new Author();
		author.setIdAuthor(bookDto.getAuthor());
		book.setAuthor(author);

		Genre genre = new Genre();
		genre.setIdGenre(bookDto.getGenre());
		book.setGenre(genre);

		Publisher publisher = new Publisher();
		publisher.setIdPublisher(bookDto.getPublisher());
		book.setPublisher(publisher);

		Collections collection = new Collections();
		collection.setIdCollection(bookDto.getCollection());
